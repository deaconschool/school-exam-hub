...
const ExcelExport = ({ classFilter = '', teacherId = '' }: ExcelExportProps) => {
  ...
  // Force LTR direction permanently for admin export - never restore RTL
  useEffect(() => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setSuccess(`Successfully exported data for ${exportData.length} students`);
    } catch (err) {
  ...
  const createExcelExport = async (data: ExportStudent[], className: string) => {
    ...
    const uniqueTeachers = allTeachers.length > 0 ? allTeachers : ['Not graded yet'];
    ...
    uniqueTeachers.forEach(teacherName => {
      headerRow.push(`${teacherName} - Delivery`);
      headerRow.push(`${teacherName} - Pronunciation`);
      headerRow.push(`${teacherName} - Group Performance`);
      headerRow.push(`${teacherName} - Total`);
    });
    ...
    summaryRow.push(`Class Average: ${Math.round(overallAverage * 100) / 100}`);
    ...
    const fileName = `Grading_Results_${className}_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-green-700 space-y-1">
            <li>• Select class to export grading results</li>
            <li>• All teacher grades for students in this class will be exported</li>
            <li>• The file includes averages and statistics</li>
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedClass} onValueChange={setSelectedClass}>
            <SelectTrigger>
              <SelectValue placeholder="Select class to export results" />
            </SelectTrigger>
            <SelectContent>
        ...
        >
          ...
          {isExporting ? 'Exporting...' : 'Export Class Results'}
        </Button>
