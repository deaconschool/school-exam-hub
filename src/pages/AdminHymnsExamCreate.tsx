...
const AdminHymnsExamCreate: React.FC = () => {
  ...
  const checkForOtherActiveExams = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setOtherActiveExams(result.data);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.isActive) {
        // Note: This can be implemented later if needed
        // For now, we'll just continue with the creation
      }

      // Create Hymns exam data matching the new table structure
      const examData = {
      ...
      const result = await AdminService.createHymnsExam(examData);

      if (result.success) {
        alert('Hymns exam created successfully!');
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Create Hymns Exam</h1>
              <p className="text-gray-600 text-sm sm:text-base">School-wide exam for all teachers and students</p>
            </div>
          </div>
          <button
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-6">
              ...
              <ul className="space-y-2 text-red-700">
                {otherActiveExams.map(exam => (
                  <li key={exam.id} className="bg-white p-3 rounded border border-red-200">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      ...
                      <span className="text-sm bg-red-100 text-red-800 px-2 py-1 rounded w-fit">
                        Currently Active
                      </span>
                    </div>
                  </li>
...
export default AdminHymnsExamCreate;